...
async function registrar() {
    ...
    if (!userVal || !passVal) return alert("Completa todos los campos");
    ...
    try {
        ...
        if (res.ok) {
            displayResultado.innerText = `✅ ${data.mensaje}. Ahora puedes iniciar sesión.`;
            displayResultado.style.color = "#28a745";
            displayResultado.parentElement.style.backgroundColor = "#d4edda";
            // Limpiar inputs
            ...
            displayResultado.innerText = `❌ Error: ${data.error}`;
            displayResultado.style.color = "#dc3545";
            displayResultado.parentElement.style.backgroundColor = "#f8d7da";
        }
    } catch (error) {
        displayResultado.innerText = "⚠️ Error de conexión.";
    }
}
